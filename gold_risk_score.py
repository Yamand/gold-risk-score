"""
Gold Risk Score — daily composite 0-1 score from OKX public candlestick data
for XAUT-USDT (Tether Gold, a token backed 1:1 by physical gold, quoted in
USDT). Tracks spot gold closely.

DATA SOURCE HISTORY (why OKX, not Binance or Stooq):
  - Stooq's free XAUUSD CSV was the original choice but turned out
    unverifiable/unreliable from the dev environment used to build this.
  - Binance's XAUTUSDT pair was tried next (same proven pattern as the BTC
    script) but has less historical depth than OKX's listing of the same
    token.
  - OKX's XAUT-USDT has more history, so that's the final source here.
...
local run's printed date range and CSV/JSON shape before relying on it.
...
Usage:
    python gold_risk_score.py            # fetch full history, recompute, write data/gold_risk_history.json
    python gold_risk_score.py --update   # fetch only recent candles and merge (fast daily run)
"""
...
PRICES_FILE = DATA_DIR / "gold_prices_raw.json"
...
# Fixed anchor for the log-regression day-count axis: the Nixon Shock
# (1971-08-15, end of dollar/gold convertibility), since the underlying
# asset is still physical gold even though it's sourced via a tokenized
# proxy here. Must never change once scores have been computed and stored,
# or historical composite_score values become inconsistent with new ones.
REGRESSION_ANCHOR = pd.Timestamp("1971-08-15")
...
def main():
    ...
    if not rows:
        raise RuntimeError(
            ...
            f"exists at https://www.okx.com/trade-spot/xaut-usdt) or the API is unreachable "
