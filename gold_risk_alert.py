"""
Gold Risk Score — daily Telegram alert.

Reads the latest row from data/gold_risk_history.json (written by
gold_risk_score.py) and pushes a formatted summary to Telegram. Meant to run
right after `gold_risk_score.py --update` in the daily GitHub Actions
...
btc_risk_alert.py — one bot, two daily messages (BTC then gold).

Requires two env vars (set as GitHub Actions secrets — the SAME ones used
by btc_risk_alert.py, no separate bot needed):
...
# gold_risk_score.py (values: "sell1", "sell2", "sell3").
...
def load_latest() -> dict:
    if not HISTORY_FILE.exists():
        print(f"ERROR: {HISTORY_FILE} not found — run gold_risk_score.py first.", file=sys.stderr)
        sys.exit(1)
...
def build_message(row: dict) -> str:
    ...
    lines = [
        f"\U0001f947 Gold Risk Score — {row['date']}",
        "",
        f"Score:  {score:.2f}  [{zone}]",
        f"Price:  {fmt_usd(price)}  (XAUT/USDT)",
        "",
        f"\u2192 {action}",
    ]

    if tier in SELL_TIERS:
        lines.append("   \u26a0\ufe0f Only if XAUT holdings \u2265 $500 — check before acting")
